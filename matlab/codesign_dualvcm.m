%% codesign_dualvcm.m - VCM N개 (힘 N배, 코일질량 N배, 구조질량 공유) γ 효과
%  DCP + Al7075, d_c=0.5, J<=10.2.  N=1,2,3 비교.
%  ※ 전제: 각 코일 독립 2A 구동(2채널). 2A가 총합 한도면 이득 없음(주석).
clear; clc; rng(7);
P.Cs=[489.4659,164.0726,-123.9167,-262.7587,-0.7897,7.3889,35.6957,-8.9347,-21.3500,28.9057];
P.I=2; P.Jmax=10.2; P.SIG=0.2; P.KT=1.7; P.mi=0.5;
...
fprintf('=== VCM 개수 N에 따른 γ (DCP+Al7075, d_c=0.5, J<=10.2, 각 코일 2A) ===\n');
fprintf('%-6s%8s%11s%9s%9s%10s%8s\n','N_vcm','gamma','stroke[mm]','f1[Hz]','Kf합','m_coil합[g]','판정');
fprintf('%s\n',repmat('-',1,62));
for N=[1 2 3]
  P.N=N;
  [g,r]=solveDCP(m,P,opts,18);
  vd='미충족'; if g>=1.0, vd='충족 OK'; end
  fprintf('%-6d%8.3f%11.3f%9.1f%9.2f%10.1f%8s\n', N, g, r.x*1e3, r.f1, r.Kf, r.mcoil*1e3, vd);
end
fprintf('\n주: 구조질량(플랫폼+유연기구)은 공유 -> N↑ 시 m_eff 분모에서 코일 비중↑,\n');
fprintf('    K_f/m_eff가 코일-단독 한계(만족)로 접근. 단 "2A=코일당" 전제.\n');
...
function r=perfDCP(x,m,P)
  ...
  Kf=P.N*v.Kf; mcoil=P.N*v.mcoil;                 % VCM N개: 힘·코일질량 N배
  [Kp,info]=flexA(L,t,b,a,a,m.E); Keff=Kp;
  Mout=m.rho*a*b*a+mcoil; Mi=P.mi*m.rho*a*b*a;     % 구조질량은 공유(N배 아님)
  K2=[4*Kp,-2*Kp;-2*Kp,2*Kp]; M2=diag([Mi,Mout]);
  fn=sort(sqrt(abs(eig(K2,M2)))/(2*pi)); f1=fn(1);
  xs=Kf*P.I/Keff; g=x(8);
  r.Keff=Keff;r.f1=f1;r.x=xs;r.Kf=Kf;r.mcoil=mcoil;r.By1=v.By1;r.VCMd=v.VCMd;r.dcs=v.dcs;r.wc=v.wc;
