...
\subsection{최종 권장 설계}
모든 물리(발열$\cdot$피로$\cdot$응력집중$\cdot$행렬법$\cdot$FEMM)를 반영한 권장 설계(표~\ref{tab:final})는 \textbf{DCP + Al7075, $d_c=0.5$\,mm(스펙 준수)}이다. $\gamma=0.93$으로 변위$\cdot$공진이 약 7\,\% 미달하며, \textbf{피로응력}($101$\,MPa$=0.2S_y$)이 binding 제약이다. DCP의 응력 절감(각 단 $x/2$) 덕에 단일 대비 직진성을 확보하면서도 경계에 근접한다.

\begin{table}[h]\centering\caption{최종 권장 설계 (DCP + Al7075, $d_c=0.5$).}\label{tab:final}\footnotesize
\begin{tabular}{ll}\toprule
항목 & 값 \\\midrule
$\gamma$ (충족률) & 0.93 \\
변위 @2A & 0.93\,mm (목표 $>1$) \\
1차 공진 $f_1$ & 92.6\,Hz (목표 $>100$) \\
유연기구 & $L\!=\!19.6,t\!=\!0.23,b\!=\!5$\,mm; stage $15$\,mm \\
$K_{\text{eff}}$, $M_{\text{eff}}$ & 2.0\,N/mm, 7.8\,g \\
VCM & $r_m\!=\!7.4,h_{y1}\!=\!4.0,t_g\!=\!1.8$\,mm, $\varnothing20$ \\
$B_g$, $K_f$, $m_{\text{coil}}$ & 597\,mT, 1.04\,N/A, 3.1\,g \\
$J$ (전류밀도) & 10.2\,A/mm$^2$ \\
피로응력 & 101\,MPa $=0.2S_y$ (binding) \\\bottomrule
\end{tabular}\end{table}

\begin{figure}[t]\centering
\includegraphics[width=0.82\linewidth]{fig_v2_final.png}
\caption{최종 설계 KPI: 변위$\cdot$공진 모두 목표의 $\approx93\%$($\gamma=0.926$).}\label{fig:final}
\end{figure}

\subsection{실현가능성 종합}
표~\ref{tab:verdict}는 모델 정정에 따른 결론 변화를 요약한다. 발열까지 정직하게 반영하면 본 스펙은 \textbf{불가능이 아니라 실현가능성 경계(약 $\gamma\!\approx\!0.93$--$1.0$)}에 있으며, $d_c>0.5$\,mm 스펙은 발열 한계로서 물리적으로 일관된다.

\begin{table}[h]\centering\caption{모델 정정에 따른 $d_c=0.5$ 실현가능성 변화.}\label{tab:verdict}\footnotesize
\begin{tabular}{lc}\toprule
모델 단계 & $\gamma$ ($d_c=0.5$) \\\midrule
단순식 $24EI/L^3$ (초기, 오류) & 0.92 (불가) \\
검증 행렬법, 단일 평행판 & 1.02 (충족) \\
검증 행렬법, DCP(직진성) + Al7075 & 0.93 (경계) \\\bottomrule
\end{tabular}\end{table}
...
\begin{minipage}{0.49\linewidth}\centering\includegraphics[width=\linewidth]{fig11_step.png}\\{\small(a) 스텝 응답}\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}\centering\includegraphics[width=\linewidth]{fig12_scan.png}\\{\small(b) 스캐닝 추종}\end{minipage}
\caption{폐루프 제어(대표 거동, 전류포화 포함).}\label{fig:dyn}
\end{figure}

\section{결론}
VCM 구동 1자유도 DCP 유연 스테이지를 \emph{공동최적화}로 설계하고, \emph{전수감사}로 모델을 정정하였다. (i) 유연기구는 검증된 컴플라이언스 행렬법으로 모델링해야 하며 단순 보 강성식은 비관적이다. (ii) 코일 발열(전류밀도) 제약이 본질적이며, 이를 반영하면 최적 선경이 $d_c\approx0.5$\,mm로 수렴하여 과제 스펙이 발열 한계와 일치함을 규명하였다. (iii) 발열$\cdot$피로$\cdot$응력집중을 모두 반영한 권장 설계(DCP+Al7075, $d_c=0.5$ 스펙 준수)는 $\gamma\approx0.93$으로 실현가능성 경계에 위치한다. 본 스펙은 모순이 아니라 물리적 frontier에서 약 7\,\% 바깥의 도전적 문제이며, 완전 충족에는 능동 냉각(전류밀도 상향) 또는 한쪽 사양의 소폭 절충이 필요하다.
